...
(defun hoppla/reload-config (&optional config)
  (interactive)
  ;; defaults to reloading this file, include the `config == "hoppla"` case
  (cond ((not config) (load-file hoppla/hoppla-file))
        ((config == "user") (load-file hoppla/user-file))
        (t (load-file hoppla/hoppla-file)))
  (hoppla/reload-theme))
...
(use-package multi-vterm
  :general
  (hoppla/leader-def "tt" '(multi-vterm-project :wk "toggle terminal"))
  (major-mode-hydra-define vterm-mode
    (:title "Vterm mode" :quit-key "q")
    ("Switch"
     (("p" multi-vterm-prev "previous")
